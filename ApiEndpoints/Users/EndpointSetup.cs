...
public static class EndpointSetup
{
    ...
    {
        app.MapGet("/users/{username}", GetUser.HandleRequest).WithTags("Users").WithOpenApi();
        app.MapGet("/users/me", GetUserMe.HandleRequest).WithTags("Users").WithOpenApi().RequireAuthorization("user");
        app.MapPost("/users/{username}", PostUser.HandleRequest).WithTags("Users").WithOpenApi();
