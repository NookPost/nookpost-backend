...
install: wipe uninstall publish
	...
	cp NookPost.service /etc/systemd/system/
	systemctl enable NookPost.service
	@echo "Installed NookPost to /opt/. Start Systemd Unit NookPost.service to run the application!"
...
install-debug: wipe uninstall publish 
	...
	if [[ ! -d $(installDirectory) ]]; then mkdir $(installDirectory); fi
	cp -r publish/* $(installDirectory) 
	cp NookPost-Debug.service /etc/systemd/system/
	systemctl enable NookPost-Debug.service
	@echo "Installed debug version of NookPost to /opt/NookPost. Start Systemd Unit NookPost-Debug.service to run the application!"
	@echo "The application uses TCP port 5000."
...
uninstall:
	...
	if [[ -f /etc/systemd/system/NookPost.service ]]; then systemctl is-active NookPost.Service && systemctl stop NookPost.service; systemctl disable NookPost.service; rm /etc/systemd/system/NookPost.service; fi
	if [[ -f /etc/systemd/system/NookPost-Debug.service ]]; then systemctl is-active NookPost.Service && systemctl stop NookPost-Debug.service; systemctl disable NookPost-Debug.service; rm /etc/systemd/system/NookPost-Debug.service; fi
